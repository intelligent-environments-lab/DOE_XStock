...
IFS='|'
{
    ...
    do
        python -m doe_xstock -d $DATABASE_FILEPATH -g $FIGURE_FILEPATH dataset $INSERT_DATASET_TYPE $INSERT_WEATHER_DATA $INSERT_YEAR_OF_PUBLICATION $INSERT_RELEASE -f $filters metadata_clustering $neighborhood_id $MAXIMUM_N_CLUSTERS -c $SAMPLE_COUNT || exit 1
    done
